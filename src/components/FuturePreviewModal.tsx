    ...
    switch (featureTitle) {
      case 'Flashcards & Memory Verses':
        return {
          ...
          bullets: ['Flip card to reveal scripture reference', 'Filter cards by topic or Testament', 'Spaced repetition study mode'],
        };
      case 'Daily Scripture Challenge':
        return {
          ...
          bullets: ['Daily XP bonus rewards', 'Global streak counter', 'Unique daily scripture topic'],
        };
      case 'Bookmarked Scripture References':
        return {
          ...
          bullets: ['Saved scripture reference list', 'Personal study notes attachment', 'Direct search across saved items'],
        };
      case 'Scripture Study Notes':
        return {
          ...
          bullets: ['Rich text notebook', 'Organized by Bible book and chapter', 'Export or print study notes'],
        };
      default:
        return {
          ...
          bullets: ['Unlock Bible Scholar badges', 'Track cumulative XP points', 'Level up from Novice to Master'],
        };
    }
  };
  ...
  return (
    <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-5">
      ...
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-1.5 rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500 hover:text-slate-900 dark:hover:text-white transition"
        >
          <X className="w-4 h-4" />
        </button>
